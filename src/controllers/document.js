...
export const uploadFiles = async (req, res) => {
  try {
    ...
    if (!teamId) {
      return res
        .status(400)
        .json({ error: "Team ID is missing from the request." });
    }
    ...
    const totalSize = files.reduce(
      (accumulator, currentObject) => accumulator + currentObject.size,
      0,
    );
    ...
    const fileDocumentIds = fileDocuments.map(
      (fileDocument) => fileDocument._id,
    );
    await Team.findByIdAndUpdate(teamId, {
      $push: { documents: { $each: fileDocumentIds } },
    });
...
export const createDocument = async (req, res) => {
  try {
    ...
    if (!teamId || !name) {
      return res
        .status(400)
        .json({ error: "Team ID and folder name are required." });
    }
...
export const updateDocument = async (req, res) => {
  try {
    ...
    if (!teamId || !documentId || !name) {
      return res
        .status(400)
        .json({ error: "Team ID, Document ID and name are required." });
    }
    ...
    res
      .status(200)
      .json({ message: "Folder details updated successfully.", folder });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Failed to create the folder." });
  }
...
export const loadMany = async (req, res) => {
  try {
    ...
    if (isAdmin) {
      ...
      documents = await Document.find({ teamId, showFile: true })
  ...
  try {
    ...
    if (userEmail) {
      ...
      if (!member) {
        ...
        return res
          .status(200)
          .json({ error: "Member not found so sending an Invite." });
      }
  ...
  try {
    ...
    if (userEmail) {
      ...
      return res
        .status(200)
        .json({
          message: `Document unshared successfully with the member with id ${memberId}`,
        });
    }
    ...
    res
      .status(200)
      .json({
        message: `Document unshared successfully with the group with id ${groupId}`,
      });
  } catch (error) {
...
export const updatePermissions = async (req, res) => {
  try {
    ...
    if (!sharedMember) {
      return res
        .status(404)
        .json({ error: "Member not found in the shared list." });
    }
    ...
    res
      .status(500)
      .json({ error: "Failed to update the document permission." });
  }
};
  ...
  try {
    ...
    if (!documentId || isFavorited === null) {
      return res
        ...
        .json({
          error:
            "Document ID, and/or favorited tag is missing from the request.",
        });
    }
...
export const updateTags = async (req, res) => {
  try {
    ...
    if (tags.length > constants.maxTags) {
      return res
        .status(400)
        .json({ message: `Only ${constants.maxTags} tags can be selected.` });
    }
...
export const deleteDocument = async (req, res) => {
  try {
    ...
    if (!documentId) {
      return res
        .status(400)
        .json({ error: "Document ID is missing from the request." });
    }
    if (!teamId) {
    ...
    if (document.type === "folder" && document.files.length > 0) {
      ...
      await Team.findByIdAndUpdate(teamId, {
        $pullAll: { documents: fileDocumentIds },
      });
    }
    ...
    res
      .status(200)
      .json({
        message: `Document with ${documentId} has been deleted successfully.`,
      });
  } catch (error) {
    console.log(error);
...
export const deleteMultipleDocuments = async (req, res) => {
  try {
    ...
    if (!Array.isArray(documentIds) || documentIds.length === 0) {
      return res
        .status(400)
        .json({ error: "Invalid or empty documentIds array" });
    }
    ...
    const foldersWithFiles = documents.filter(
      (doc) => doc.type === "folder" && doc.files.length > 0,
    );

    // If there are folders with files, delete the files first
    if (foldersWithFiles.length > 0) {
      const fileDocumentIds = foldersWithFiles.flatMap(
        (folder) => folder.files,
      );
    ...
    res
      .status(200)
      .json({ message: `${documents.length} documents deleted successfully` });
  } catch (error) {
    console.log(error);
