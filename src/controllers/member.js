...
export const createMember = async (req, res) => {
  try {
    ...
    const avatarUrlPreview =
      (req.body.avatarUrl && req.body.avatarUrl?.preview) || "";
    ...
    if (team && team.companyName !== req.body.company) {
      return res
        .status(400)
        .json({ error: "Company name should be the same as the team name." });
    }
    ...
    res
      .status(201)
      .json({ message: "A new member has been created", newMember });
  } catch (error) {
...
export const loadMany = async (req, res) => {
  try {
    ...
    if (memberIds) {
      ...
      const sharedMembers = await Member.find({
  ...
  try {
    ...
    if (!member) {
      return res
        .status(404)
        .json({
          error: "Member not found or does not belong to the specified team.",
        });
    }
...
export const updateByEmail = async (req, res) => {
  try {
    ...
    const errorMsg = error?.message || "";
    logger.error([
      "[updateByEmail -- member] Failed to update member",
      errorMsg,
    ]);
...
export const move = async (req, res) => {
  try {
    ...
    const errorMsg = error?.message || "";
    logger.error([
      "[move -- member] Failed to move the member to the group.",
      errorMsg,
    ]);
    return res
      .status(500)
      .json({ error: "Failed to move the member to the group." });
  }
};
...
export const deleteOne = async (req, res) => {
  try {
    ...
    if (!id) {
      return res
        .status(400)
        .json({ error: "Member ID is missing from the request." });
    }
    ...
    await Team.findByIdAndUpdate(member.teamId, {
      $pull: {
        members: id,
        shared: member.email,
    ...
    res
      .status(200)
      .json({ message: `Member with ${id} id has been deleted successfully` });
  } catch (error) {
    const errorMsg = error?.message || "";
...
export const deleteMany = async (req, res) => {
  try {
    ...
    if (!Array.isArray(memberIds) || memberIds.length === 0) {
      return res
        .status(400)
        .json({ error: "Invalid or empty memberIds array" });
    }
    ...
    const membersDeletedInfo = await Member.deleteMany({
      _id: { $in: memberIds },
      teamId,
    });
    ...
    res
      .status(200)
      .json({
        message: `${membersDeletedInfo.deletedCount} members deleted successfully`,
        memberIds: firebaseIds,
      });
  } catch (error) {
    const errorMsg = error?.message || "";
    logger.error([
      "[deleteMany -- member] Failed to delete members.",
      errorMsg,
    ]);
