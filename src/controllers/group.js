...
export const createGroup = async (req, res) => {
  try {
    ...
    await Group.updateMany( { members: { $in: members } }, { $pull: { members: { $in: members } } } );
    // Remove members from their existing groups, if any
    await Member.updateMany({ _id: { $in: members }, groupId: { $exists: true } }, { $unset: { groupId: 1 } });
    ...
    await Member.updateMany({ _id: { $in: members } }, { groupId: newGroup._id });

    res.status(201).json({ message: 'A new group has been created', group: newGroup });
  } catch (error) {
    console.log(error);
...
export const loadMany = async (req, res) => {
  try {
    ...
    const groups = await Group.find({ teamId }).populate('members', '_id name email avatarUrl');
...
export const loadOne = async (req, res) => {
  try {
    ...
    if (!group) {
      return res.status(404).json({error: 'Group not found or does not belong to the specified team.'})
    }
...
export const updateGroup = async (req, res) => {
  try {
    ...
    if (!updatedGroup) {
      return res.status(400).json({error: 'Group not found or does not belong to the specified team.'})
    }
...
export const removeFromGroup = async (req, res) => {
  try {
    ...
    if (!groupId) {
      return res.status(400).json({ error: 'Group ID is missing from the request.' });
    }
    ...
    res.status(200).json({message: `Member with ${memberId} id has been removed from group`})
  } catch (error) {
    console.log(error.message);
...
export const deleteGroup = async (req, res) => {
  try {
    ...
    res.status(200).json({message: `Group with ${groupId} id has been deleted successfully`})
  } catch (error) {
    console.log(error.message);
...
export const deleteMultipleGroups = async (req, res) => {
  try {
    ...
    await Team.findByIdAndUpdate(teamId, { $pull: { groups: { $in: groupIds } } });
    // Update members' groupId to null for the deleted groupIds
    await Member.updateMany({ groupId: { $in: groupIds } }, { $set: { groupId: null } });

    res.status(200).json({ message: `${groups.deletedCount} groups deleted successfully` });
  } catch (error) {
    console.log(error.message);
